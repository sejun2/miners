...
global movingMutex := 0

; get mineral image file by string
getMineralImageFile(mineralName){
    result := "images\mineral\" . %mineralName% . ".png"
...
; 로랜시아 광산 4층
class Rolancia4FMiner extends IMiner{
    ...
    inventoryCount := get_inven()

    __new(mineralList){
        this.mineralList := mineralList

        this.mapSectionXY[0] := [-1, 53]
        this.mapSectionXY[1] := [-33, 13]
        this.mapSectionXY[2] := [-30, -34]
        this.mapSectionXY[3] := [2, -24]
        this.mapSectionXY[4] := [32, -33]
        this.mapSectionXY[5] := [50, 6]
        this.mapSectionXY[6] := [32, -33]
        this.mapSectionXY[7] := [18, 2]
    }    

    mine(){
        GuiControl, , text_behaviour, 행동 상태 : Mine
        ; mineralImagePath := getMineralImageFile(mineralList[%A_Index%])
        ;MsgBox, %mineralImagePath%
        ...
        loop{
            ...
            ImageSearch, FoundX, FoundY, 0, 0, 800, 600, %A_ScriptDir%\muro.bmp
            if ErrorLevel = 2
            {
                MsgBox, 이미지가 없습니다
                ExitApp
            ...
            {    
                GuiControl, , text_behaviour, 행동 상태 : 광물 찾는중
                this.move()
            }
            else{
                GuiControl, , text_behaviour, 행동 상태 : 광물 찾기 성공 x: %FoundX% y: %FoundY%
                ;msgbox, 찾기성공
                WinActivate, %WindowTitle%
                PostClick(FoundX-4, FoundY-4)
...
SendSpaceTwice()
                ...
                ; 스페이스 입력 두 번 보내기
                ; PostMessage, 0x100, 0x20, 0, , A ; 키 다운
                ; PostMessage, 0x101, 0x20, 0, , A ; 키 업
    ...
    move(){
        if(movingMutex = 1) return

        movingMutex := 1
        posX := this.mapSectionXY[mapSectionIndex][1]
        posY := this.mapSectionXY[mapSectionIndex][2]
    
    length := this.mapSectionXY.maxIndex()

        Click_CurrentMiniMapPos(posX, posY)
        mapSectionIndex++
        ;msgBox, mapSectionIndex : %mapSectionIndex%

        if(mapSectionIndex > length){
            mapSectionIndex := 0
        }    
...
; 로랜시아 광산 1층
class Rolancia1FMiner extends IMiner{
    ...
    __new(mineralList){
        ...
        ;67,87  40,75   51,30  78,43 
        ; 90, 55  103.26  120,68  127,47  
        ;143,29  165, 38  189,45  235,56  238,34   226,91  258, 90


        this.mapSectionXY[0] := [-83, 26]
        this.mapSectionXY[1] := [-110, 14]
        this.mapSectionXY[2] := [-99, -31]
        this.mapSectionXY[3] := [-72,-18]
        this.mapSectionXY[4] := [-60, -6]
        this.mapSectionXY[5] := [-47, -35]
        this.mapSectionXY[6] := [-30, 7]
        this.mapSectionXY[7] := [-23, -14]
        this.mapSectionXY[8] := [-7, -32]
        this.mapSectionXY[9] := [15, -23]
        this.mapSectionXY[10] := [39, -16]
        this.mapSectionXY[11] := [85, -5]
        this.mapSectionXY[12] := [88, -27]
        this.mapSectionXY[13] := [76, 30]
        this.mapSectionXY[14] := [108, 29]
        this.mapSectionXY[15] := [76, 30]
        this.mapSectionXY[16] := [88, -27]
        this.mapSectionXY[17] := [85, -5]
        this.mapSectionXY[18] := [39, -16]
        this.mapSectionXY[19] := [15, -23]
        this.mapSectionXY[20] := [-7, -32]
        this.mapSectionXY[21] := [-23, -14]
        this.mapSectionXY[22] := [-30, 7]
        this.mapSectionXY[23] := [-47, -35]
        this.mapSectionXY[24] := [-60, -6]
        this.mapSectionXY[25] := [-72,-18]
    }    
...
class MinerFactory {
    ...
    __new(){
        msgbox, minerfactory called
    }

    create(mapName, mineralList){
        return new Rolancia1FMiner(mineralList)

        ;     mapName :=SubStr(mapName, 1)

        ;     if(mapName = 로랜시아 광산 1층){
        ;         result := new RolanciaMiner(mineralList)
        ;         return result
        ;     }
        ;     else if(mapName = 에필로리아 광산 1층){
        ;         return new EpiloriaMiner(mineralList)
        ;     }
        ; else if(mapName = 크로노시스 광산 1층){
        ;         msgbox, 크로노시스 광산 1층 called
        ;         return new RolanciaMiner(mineralList)
        ; }
        ; else if(mapName = 크로노시스 광산 2층){
        ;         return new RolanciaMiner(mineralList)
        ; }    
    }    
}
...
is_moving(){
    isMoving := ReadMemory(ReadMemory(ReadMemory(0x0058eb04)+0x18)+0x174)
    return isMoving
}
...
GetMapNumber(){
    ...
    if(ReadMemory(jelanCoreAdd + 0x0018EB6C) = 0) {
        ...
        PostMessage, 0x100, 0x56, 30    80193,,%WindowTitle%
...
Click_CurrentMiniMapPos(A,B){
    ...
    ;PostMove(CurrentMiniMapPosX, CurrentMiniMapPosY)
...
SendSpaceTwice()
{
    ...
    ; 스페이스바를 두 번 입력합니다.
    PostMessage, 0x100, 0x20, 0, , %WindowTitle% ; WM_KEYDOWN
    PostMessage, 0x101, 0x20, 0, , %WindowTitle% ; WM_KEYUP
    Sleep, 150
    PostMessage, 0x100, 0x20, 0, , %WindowTitle% ; WM_KEYDOWN
    PostMessage, 0x101, 0x20, 0, , %WindowTitle% ; WM_KEYUP
}
...
PostClick(MouseX,MouseY){
    ...
    SendMessage, 0x200, 0, %MousePos% ,,%WindowTitle%
    SendMessage, 0x201, 1, %MousePos% ,,%WindowTitle%
    SendMessage, 0x202, 0, %MousePos% ,,%WindowTitle%
}
...
RefleshWindowList(TitleNumber){
    ...
    GuiControl,, WindowTitle%TitleNumber%_Selected,|
    Loop, %jElancia_Count%{
        temp_Variable := WindowTitle%A_Index%
        GuiControl,, WindowTitle%TitleNumber%_Selected, %temp_Variable%
    }
}
